jQuery(document).ready(function () {
    var options = {
        onLoad: function () {
            $('#messages').text('Comece digitando a senha');
        },
    ...
    $('.ValidaSenha').pwstrength(options);
});
...
(function ($) {
    ...
    var options = {
        ...
        minChar: 8,
        errorMessages: {
            password_to_short: "A senha é pequena demais",
            same_as_username: "Sua senha não pode ser igual seu usuário"
        },
        scores: [17, 26, 40, 50],
    ...
    setProgressBar = function ($el, score) {
        ...
        if (score < options.scores[0]) {
            $(".progress").addClass("progress-danger").removeClass("progress-warning").removeClass("progress-success");
            $(".progress").find(".bar").css("width", "5%");
            if (options.showVerdicts) {
                $verdict.text(options.verdicts[0]);
            }
        } else if (score >= options.scores[0] && score < options.scores[1]) {
            $(".progress").addClass("progress-danger").removeClass("progress-warning").removeClass("progress-success");
            $(".progress").find(".bar").css("width", "25%");
            if (options.showVerdicts) {
                $verdict.text(options.verdicts[1]);
            }
        } else if (score >= options.scores[1] && score < options.scores[2]) {
            $(".progress").addClass("progress-warning").removeClass("progress-danger").removeClass("progress-success");
            $(".progress").find(".bar").css("width", "50%");
            if (options.showVerdicts) {
                $verdict.text(options.verdicts[2]);
            }
        } else if (score >= options.scores[2] && score < options.scores[3]) {
            $(".progress").addClass("progress-warning").removeClass("progress-danger").removeClass("progress-success");
            $(".progress").find(".bar").css("width", "75%");
            if (options.showVerdicts) {
                $verdict.text(options.verdicts[3]);
            }
        } else if (score >= options.scores[3]) {
            $(".progress").addClass("progress-success").removeClass("progress-warning").removeClass("progress-danger");
